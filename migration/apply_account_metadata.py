#!/usr/bin/env python3
"""Idempotently create Account custom columns in AccountMigration solution."""
...
def schema_name(logical: str) -> str:
    """crcc0_readyforai -> crcc0_Readyforai style schema for custom attributes."""
    if "_" not in logical:
        return logical
...
def attribute_exists(logical_name: str) -> bool:
    env = os.environ["DATAVERSE_URL"].rstrip("/")
    token = get_token()
    url = (
        f"{env}/api/data/v9.2/EntityDefinitions(LogicalName='account')"
...
def post_attribute(body: dict) -> str:
    env = os.environ["DATAVERSE_URL"].rstrip("/")
    token = get_token()
    req = urllib.request.Request(
        f"{env}/api/data/v9.2/EntityDefinitions(LogicalName='account')/Attributes",
    ...
    try:
        ...
        raise RuntimeError(f"Create attribute failed ({e.code}): {detail}") from e
...
def main():
    ...
    for field in field_map["customFields"]:
        ...
        def do_create(f=field):
            post_attribute(build_attribute_payload(f))
            time.sleep(3)
